...
public class JavaArchiveUtils {
	 ...
	 * @param outdir Output directory - if this is null the extract in the same folder where .tar.gz file is
	 */
	public static void extractTarFile(File tarfile, File destFolder ){
        ...
        try{
        	if(!tarfile.exists()){
     		...
     		if(destFolder == null){
     			destFolder = tarfile.getParentFile();	
     		}
            ...
            try{
	            ...
	            if(jTarInputStream != null){
	            	jTarInputStream.close();
	            }
            }catch(Exception exp){
            	throw new RuntimeException(exp);
            }
